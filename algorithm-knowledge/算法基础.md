...
### 中等类型
**先循环物品，再循环容量（背包），就是不考虑顺序；先循环容量，再循环物品（背包），就是考虑所选物品的顺序。正序遍历容量，允许重复选择物品；逆序遍历容量，防止重复计算。**
...
class Solution {
   public int knapsack(int[] weight, int[] value, int capacity) {
      ...
      // 外层遍历物品 → 组合数（不考虑顺序）即 3 [1,2] [2,1] 只能算一种
      for (int i = 0; i < weight.length; i++) {
         // 正序遍历容量，允许重复选择。j 是从小到大依次递增的。当计算 dp[j] 时，dp[j - weight[i]] 可能之前由小到大过程中计算过了，这就意味着物品可能被重复选择了。
