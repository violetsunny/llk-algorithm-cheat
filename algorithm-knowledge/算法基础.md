...
q.push(i);//在头部添加元素 addFirst()
q.addFirst(i);//在头部添加元素
q.add(i);//末尾放入i addLast()
q.offerFirst(i);//在头部添加元素
...
queue.push(i);//在头部添加元素 addFirst()
queue.add(i);//也是往后放，就是尾添加,addLast()
queue.offer(i);//末尾放入i,addLast()
queue.peek();//取出头部元素
queue.peekLast();//取出尾部的元素
queue.pop();//取出头部元素并移除
queue.poll();//取出头部元素并移除
queue.pollLast();//取出尾部的元素并移除
...
Arrays.sort(bak, (x, y) -> (y & 1) - (x & 1));//快排 O(n log n)
...
int index = x & -x; //index是x的二进制表示中最低位那个 1
x & -x == x; //说明x是2的幂

a & b == 0;//说明a和b没有交集，!=0是有交集

(a + b) % n == ((a % n) + (b % n)) % n;//a+b和n的余数就是 a和n的余数 + b和n的余数 再和n取余
...
{2,3} ∩ {1,2} = {2}
1 1 0 0 {2,3}
              & 0 1 0 0 {2} !=0就是有交集，=0就是没有交集
0 1 1 0 {1,2}

将集合合并，可以按位或。就是并集：A∪B
位运算：a ∣ b，
{3,4} ∪ {2,3} = {2,3,4}
1 1 0 0 0 {3,4}
                | 1 1 1 0 0 {2,3,4}
...
如果要从大到小枚举 s 的所有子集sub（从s 枚举到空集∅），可以这样写：
int sub = s;
do {
// 处理 sub 的逻辑
sub = (sub - 1) & s;
...
public class FastRowSort {
    ...
    private void qsort(int[] arr, int begin, int end) {
        ...
        int partition = partition(arr, begin, end);//找到基准点
        qsort(arr, begin, partition - 1);//基准点左边排
        qsort(arr, partition + 1, end);//基准点右边排
    }
...
class Solution {
    ...
    public int merge(int[] nums, int l1, int r1, int l2, int r2) {
        ...
        while (i <= r1 && j <= r2) {
            if (nums[i] > nums[j]) {
                count = count + (l2 - i);//如果i大于j,说明i到r1位置的都是比j大。
                temp[k++] = nums[j++];
            } else {
                temp[k++] = nums[i++];
            }
    ...
    private void merge(int[] nums, int left, int mid, int right) {
        ...
        while (i <= mid && j <= right) {
            if (nums[i] <= nums[j]) {
                ...
                tempIndex[k] = index[i];
               // 虽然i比j小，但是i比j之前都要大；
               // 因为如果j往右挪动了而i没有，说明j之前的都要小于当前i(也就是mid+1到j-1的位置)，所以就是j-mid-1;
               // 当然如果没有移动，减了还是0;
                ans[index[i]] += (j - mid - 1);
                k++;
                i++;
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            while (l < r) {
                ...
                } else if (nums[l] + nums[r] + target > 0) {//指针移动
                    r--;
                } else {
                    l++;
                }
    ...
    public int maxArea(int[] height) {
        ...
        while (l < r) {
            ...
            if (height[r] > height[l]) {
                l++;//因为height[r]大于height[l]，要想max更大，只能l往右移动找到更大的边
            } else {
                r--;
            }
        }
...
class Solution {
    public int lengthOfLongestSubstring(String s) {
        boolean[] ss = new boolean[128];//字符ASCII码最大到128
        int max = 0;
        int j = 0;//j是左侧，i往右移动
...
### 中等类型
**先循环物品，再循环容量（背包），就是不考虑顺序（可以理解为：随便选物品，不看顺序。就想象成物品放在前面，背包在后面的场景）；
先循环容量（背包），再循环物品，就是考虑所选物品的顺序（可以理解为：固定住背包容量大小，物品不同的顺序结果不一样。背包放在前面，物品在后面要往背包里放）。
正序遍历容量（背包），允许重复选择物品（可以理解为：从小到大排，小的物品可以拿多次）；
逆序遍历容量（背包），防止重复计算（可以理解为：从大到小排，大的物品只能拿一次）。**
...
class Solution {
   public int knapsack(int[] weight, int[] value, int capacity) {
      ...
      for (int i = 0; i < weight.length; i++) {
         // 逆序遍历容量，防止重复计算。dp[j - weight[i]] 是由大到小计算，这就意味着物品之前没被计算过，也就是只被选择一次。
...
class Solution {
   public int knapsack(int[] weight, int[] value, int capacity) {
      ...
      // 外层遍历物品 → 组合数（不考虑顺序）即 3 [1,2] [2,1] 只能算一种
      for (int i = 0; i < weight.length; i++) {
         // 正序遍历容量，允许重复选择。j 是从小到大依次递增的。当计算 dp[j] 时，dp[j - weight[i]] 可能之前由小到大过程中计算过了（例如：5-3 和 4-2），这就意味着物品可能被重复选择了。
...
class Solution {
   public int knapsack(int[] weight, int[] value, int capacity) {
      ...
      // 外层遍历容量 → 排列数（考虑顺序）即 3 [1,2] [2,1] 算两种
      for (int i = 1; i <= capacity; i++) {
         for (int j = 0; j < weight.length; j++) {// 重复选择
            if (i >= weight[j]) {
...
class Solution {
    public int coin(int[] coins, int amount) {
       ...
       // 先循环容量、再循环物品（考虑顺序）也就是 3 [1,2] [2,1] 是两种
...
//        }
       ...
       // 先循环物品、再循环容量（不考虑顺序）即 3 [1,2] [2,1] 只能算一种
...
class Solution {
    public int numSquares(int n) {
        ...
        // 完全背包优化：外层遍历物品（平方数）→ 组合数（不考虑顺序）即 3 [1,2] [2,1] 只能算一种，内层正序遍历容量
        for (int s = 1; s * s <= n; s++) {      // 遍历所有可能的平方数
            int square = s * s;                 // 当前平方数
            for (int j = square; j <= n; j++) { // 正序遍历容量，允许重复
...
class Solution {
    public int combinationSum4(int[] nums, int target) {
        ...
        dp[0] = 1;
        // 外层遍历容量 → 排列数（考虑顺序） 也就是 3 [1,2] [2,1] 是两种
        for (int i = 1; i <= target; i++) {
            for (int num : nums) {// 重复选择
                if (i >= num) {
                    dp[i] = dp[i] + dp[i - num];
                }
            }
...
class Solution {
   ...
   private int robRange(int[] nums, int start, int end) {
      int[] dp = new int[end];
      dp[start + 0] = nums[start];//一定要从start开始
...
class Solution {
    ...
    public int maxProfit(int[] prices) {
        ...
        for (int i = 0; i < prices.length; i++) {
            minprice = Math.min(minprice,prices[i]);//选出最小值
            maxprofit = Math.max(maxprofit,prices[i] - minprice);//计算最大差值
        }
        return maxprofit;
    }
}
...
class Solution {
    ...
    public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; ++i) {
            ans = ans + Math.max(0, prices[i] - prices[i - 1]);//大于0，就是卖出有利可图，就进行操作
        }
        return ans;
    }
}
...
class Solution {
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        for (int i = 0; i < gas.length; i++) {
            s += gas[i] - cost[i];//需要油量，就是计算从头到尾是不是>0，能满足一圈
            if (s < min) {//如果越来越小，说明当前起点不合适，要往后挪动
                min = s;//更新最小油量
                index = i + 1;//更新需要的起点，记录从哪个点开始s油量开始变多，如果最后>0了，说明这个点就是起点
            }
        }
...
class Solution {
    ...
    public int longestIncreasingPath(int[][] matrix) {
        ...
        while (!queue.isEmpty()) {//广度优先搜索(BFS)
            ++ans;
            int size = queue.size();
            for (int i = 0; i < size; ++i) {//因为四个方向，可能有多个
                int[] cell = queue.poll();
