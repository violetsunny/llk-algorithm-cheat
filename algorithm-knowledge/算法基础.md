...
class Solution {
    ...
    private boolean backtrack(char[][] board, int i, int j, char[] words, int k) {
        ...
        board[i][j] = '#';//标记搜过
        //上下左右搜索
        boolean res = backtrack(board, i + 1, j, words, k + 1) 
                || backtrack(board, i, j + 1, words, k + 1) 
                || backtrack(board, i - 1, j, words, k + 1) 
                || backtrack(board, i, j - 1, words, k + 1);
...
class Solution {
    public int coin(int[] coins, int amount) {
        ...
        Arrays.fill(dp, max);//初始化最大值，最大肯定amount+1，就是都是面值1
...
### 数字 1 的个数
公式：$cur = (n / bit) \% 10 $, $low = n \% bit$, $high = n / bit / 10$
- cur > 1  => $(high + 1) * bit$
