...
class Solution {
    ...
    private TreeNode build(int[] preorder, int[] inorder, int s1, int e1, int s2, int e2) {
        ...
        root.left = cnt > 0 ? build(preorder, inorder, s1 + 1, s1 + cnt, s2, i - 1) : null;//前序的左边，中序的左边
        root.right = i < e2 ? build(preorder, inorder, s1 + cnt + 1, e1, i + 1, e2) : null;//前序的右边，中序的右边
        return root;
    }
}
```
...
class Solution {
    ...
    /**
     * 判断字符串是否与模式串匹配
     * 1. p[j]='.' 或 s[i] : dp[i][j]=dp[i-1][j-1];
     * 2. p[j]='*' 
     *    p[j-1] != s[i] : dp[i][j]=dp[i][j-2]; //*代表0个，就是i和j-2比,例如a 和 as*
     *    p[j-1] = s[i] : dp[i][j]=dp[i][j-1]; //*代表1个，就是i和j-1比,例如aa 和 aa*
